...
class attention(nn.Module):
    def __init__(self, size_layer, embedded_size, size, output_size,
        ...
        self.linear1 = nn.Linear(size, embedded_size)
...
class ByteNet(nn.Module):
    ...
    def __init__(self, num_channels=6, input_channel=5,num_sets=1, dilation_rates=[1,2,4,8,16,1,2,4,8,16], kernel_size=1, block=modules.ResidualBlock, causal=True):
        super(ByteNet, self).__init__()
        self.seq = nn.Sequential()
        for s in range(num_sets):
            for r in dilation_rates:
                self.seq.add_module('block%s_%s' % (s, r),
                                block(num_channels, input_channel,kernel_size=kernel_size, dilation=r, causal=causal))
        self.logits=nn.Linear(num_channels,num_channels)
